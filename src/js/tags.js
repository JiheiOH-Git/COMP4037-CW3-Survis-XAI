    ...
    function parseEntries(options) {
        ...
        $.each(bib.entries, function (id, entry) {
            ...
            if (options.field === 'warning') {
                ...
                var parsedTags = bibUtil.parseField(
                    entry[options.field], options.field, bib.tagCategories
                );
                if (options.field === 'keywords' && entry['paper_type']) {
                    var unknownTypeIndex = parsedTags.indexOf('type:?');
                    var paperType = 'type:' + entry['paper_type'].toLowerCase();
                    if (unknownTypeIndex >= 0) {
                        parsedTags[unknownTypeIndex] = paperType;
                    } else if (parsedTags.indexOf(paperType) < 0) {
                        parsedTags.push(paperType);
                    }
                }
                bib.parsedEntries[id][options.field] = parsedTags;
            }
        });
    }
